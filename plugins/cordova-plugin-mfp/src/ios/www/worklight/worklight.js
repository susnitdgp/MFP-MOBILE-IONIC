...
WL.Logger = (function (jQuery) {
  ...
  var __getStateDefaults = function() {
    ...
    return {
      ...
      pkg : '',
      allowlist : [],  // @deprecated since version 6.2; use filters instead
      blocklist : [],  // @deprecated since version 6.2; use filters instead
      filters : udf,
  ...
  var __setState = function (options) {
    ...
    state = {
        ...
        allowlist : options.allowlist || state.allowlist,  // @deprecated in 6.2; use filters instead
        blocklist : options.blocklist || state.blocklist,  // @deprecated in 6.2; use filters instead
  ...
  //stateLevel can be an Array (allowlist of levels), a string (e.g. 'warn') or a number (200)
  var __checkLevel = function (currentPriority, stateLevel) {

    if ($.isArray(stateLevel)) {

      return  (//Check if current is allowlisted (state)
          stateLevel.length > 0 &&
  ...
  var __checkLists = function (pkg, allowlistArr, blocklistArr) {

    return (//Package inside allowlist
        ($.isArray(allowlistArr) && allowlistArr.length > 0 && !__insideArray(pkg, allowlistArr)) ||

        //Package inside Blocklist
        ($.isArray(blocklistArr) && blocklistArr.length > 0 && __insideArray(pkg, blocklistArr))
    );
  };
  ...
  if(WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS8 || WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS || WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWSPHONE8){
    ...
    if (!(__checkNativeEnvironment() || _isWinUWPEnv())) {
      if (!state.enabled ||
          ...
          __checkLists(pkg, state.allowlist, state.blocklist) ||
